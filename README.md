...
基于 `FI_SAP_ECC6.xml` / `MM_SAP_ECC6.xml` / `SD_SAP_ECC6.xml` 三份配置的实际字段，对照 SAP ECC6 标准底表语义，梳理本工具可抽取的表及其对两类审计程序的支撑：

- **三单匹配**：采购（订单 / 收货 / 发票）、销售（订单 / 发货 / 开票）
- **序时账与课余表核对**：`BKPF+BSEG` 凭证明细 vs `FAGLFLEXT` / `GLT0` 余额表，及客户 / 供应商 / 总账次要索引
...
| 审计场景 | 维度 | 覆盖 | 缺口 |
|---|---|---|---|
| **三单匹配** | 采购三单 + 串联键 `EBELN/EBELP`（EKBE） | 完整 | 价格条件 `KONV/KONP` 未抽 |
| | 销售三单 + 串联键 `AUBEL/AUPOS` / `VGBEL/VGPOS`（VBFA） | 完整 | 同上 |
| **序时账 vs 课余表** | 序时账 BKPF+BSEG | 完整 | — |
| | 课余表 FAGLFLEXT/GLT0 + 次要索引 + 主数据 | 完整 | CO 模块未抽 |
| | 哈希完整性校验 | 内建 | — |

**给审计判断的提示**：① 本工具保证「该取的底表与字段都取到」，差异判定在下游工具；② 三单匹配数据齐备但价格条件缺；③ 序时账核对数据齐备但 CO 缺；④ dry-run 范围（Year=2020、Date=20200101–20201231、CoCode 列表）为占位，正式取数须改写 `<Scope>` 并保证序时账与余额表期间口径一致。
